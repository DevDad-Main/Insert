...
export const EmailClient = () => {
  ...
  const handleSelect = useCallback(() => {
    if (selectedEmailId) {
      // Mark as read
      setEmails((prev) =>
        prev.map((e) => (e.id === selectedEmailId ? { ...e, isRead: true } : e))
      );
    }
  }, [selectedEmailId]);
  ...
  const handleDelete = useCallback(() => {
    if (mode === 'VISUAL' && selectedEmailIds.length > 0) {
      ...
      setEmails((prev) =>
        prev.map((e) => (e.id === selectedEmailId ? { ...e, folder: 'trash' } : e))
      );
  ...
  const handleArchive = useCallback(() => {
    if (mode === 'VISUAL' && selectedEmailIds.length > 0) {
      ...
      setEmails((prev) =>
        prev.map((e) => (e.id === selectedEmailId ? { ...e, folder: 'archive' } : e))
      );
  ...
  return (
    <div className="h-screen w-screen bg-background flex flex-col overflow-hidden">
      ...
      <VimStatusBar
        ...
        keybindHint={
          mode === 'NORMAL'
            ? 'j/k: navigate • Enter: open • c: compose • a: AI assistant'
            : mode === 'VISUAL'
